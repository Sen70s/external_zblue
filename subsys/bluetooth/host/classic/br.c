...
void bt_hci_conn_req(struct bt_dev *hdev, struct net_buf *buf)
{
	...
	LOG_DBG("conn_req from %s type 0x%02x\n",
	       bt_addr_str(&evt->bdaddr), evt->link_type);
	conn = bt_conn_add_br(hdev, &evt->bdaddr);
	if (!conn) {
		LOG_ERR("conn_req: add_br failed - reject\n");
	...
	if (bt_accept_conn(hdev, &evt->bdaddr)) {
		...
		LOG_ERR("conn_req: accept failed\n");
		bt_conn_unref(conn);
		return;
	}
	LOG_DBG("conn_req: accepted\n");
...
int bt_br_init(struct bt_dev *hdev)
{
	...
	LOG_DBG("br_init: WRITE_SSP_MODE ret=%d\n", err);
	if (err) {
		return err;
	}
	...
	LOG_DBG("br_init: set scan activity\n");
